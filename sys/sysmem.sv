...
(
	...
	// ---- h2f AXI-3 master, HYBRID_EMU only. The comma below is inside the guard so classic builds keep the stock port list.
`ifdef HYBRID_EMU
	,
	//  enabled here (port_size_config 2'b00; stock MiSTer ties 2'b11 = DISABLED) and brought out for sys_top
...
sysmem_HPS_fpga_interfaces fpga_interfaces (
	...
	// ---- h2f passthrough: connect sysmem_lite's boundary ports to the submodule ports that feed the hps2fpga primitive
`ifdef HYBRID_EMU
...
(
	...
	// ---- h2f AXI-3 master, declared at this scope so the hps2fpga primitive references real ports rather than implicit nets
`ifdef HYBRID_EMU
...
`ifdef HYBRID_EMU
// h2f master ENABLED (port_size_config 2'b00) and fully wired
...
`else
// classic MiSTer: h2f master DISABLED -- stock stub, port_size_config 2'b11, no data ports
cyclonev_hps_interface_hps2fpga hps2fpga(
